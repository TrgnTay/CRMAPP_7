$(document).ready(function() {var formatter = new CucumberHTML.DOMFormatter($('.cucumber-report'));formatter.uri("file:src/test/resources/features/AddingMention.feature");
formatter.feature({
  "name": "Sending Mention",
  "description": "",
  "keyword": "Feature",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.scenario({
  "name": "Sending Mention functionality",
  "description": "",
  "keyword": "Scenario",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.before({
  "status": "passed"
});
formatter.step({
  "name": "User logs in with valid credentials",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.Login_StepDefinitions.user_logs_in_with_valid_credentials()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks Message button on the top dashboard",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.ClickingMessageButtonStepDefs.user_clicks_Message_button_on_the_top_dashboard()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks mention icon",
  "keyword": "When "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AddingMentionStepDefs.user_clicks_mention_icon()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User selects an email",
  "keyword": "When "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AddingMentionStepDefs.user_selects_an_email()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "Email is displayed in the message box",
  "keyword": "Then "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AddingMentionStepDefs.emailIsDisplayedInTheMessageBox()"
});
formatter.result({
  "status": "passed"
});
formatter.after({
  "status": "passed"
});
formatter.uri("file:src/test/resources/features/AddingNewUsers.feature");
formatter.feature({
  "name": "Adding New users",
  "description": "",
  "keyword": "Feature",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.scenario({
  "name": "Adding New Users Feature",
  "description": "",
  "keyword": "Scenario",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.before({
  "status": "passed"
});
formatter.step({
  "name": "User logs in with valid credentials",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.Login_StepDefinitions.user_logs_in_with_valid_credentials()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks Message button on the top dashboard",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.ClickingMessageButtonStepDefs.user_clicks_Message_button_on_the_top_dashboard()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks Add more",
  "keyword": "When "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AddingNewUsersStepDefs.user_clicks_Add_more()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks an email from the pop-up window",
  "keyword": "And "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AddingNewUsersStepDefs.user_clicks_an_email_from_the_pop_up_window()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "Email is displayed in the To: box",
  "keyword": "Then "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AddingNewUsersStepDefs.email_is_displayed_in_the_To_box()"
});
formatter.result({
  "status": "passed"
});
formatter.after({
  "status": "passed"
});
formatter.uri("file:src/test/resources/features/AttachingNewLink.feature");
formatter.feature({
  "name": "Adding New Link",
  "description": "",
  "keyword": "Feature",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.scenario({
  "name": "Adding New link Feature",
  "description": "",
  "keyword": "Scenario",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.before({
  "status": "passed"
});
formatter.step({
  "name": "User logs in with valid credentials",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.Login_StepDefinitions.user_logs_in_with_valid_credentials()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks Message button on the top dashboard",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.ClickingMessageButtonStepDefs.user_clicks_Message_button_on_the_top_dashboard()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks link icon",
  "keyword": "When "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AttachingNewLinkStepDefs.user_clicks_link_icon()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User inserts Link Text and Link URL",
  "keyword": "And "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AttachingNewLinkStepDefs.user_inserts_Link_Text_and_Link_URL()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "Link is displayed in the message box",
  "keyword": "Then "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.AttachingNewLinkStepDefs.link_is_displayed_in_the_message_box()"
});
formatter.result({
  "status": "passed"
});
formatter.after({
  "status": "passed"
});
formatter.uri("file:src/test/resources/features/SendingMessage.feature");
formatter.feature({
  "name": "Sending Message",
  "description": "",
  "keyword": "Feature",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.scenario({
  "name": "Sending Message functionality",
  "description": "",
  "keyword": "Scenario",
  "tags": [
    {
      "name": "@wip"
    }
  ]
});
formatter.before({
  "status": "passed"
});
formatter.step({
  "name": "User logs in with valid credentials",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.Login_StepDefinitions.user_logs_in_with_valid_credentials()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User clicks Message button on the top dashboard",
  "keyword": "Given "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.ClickingMessageButtonStepDefs.user_clicks_Message_button_on_the_top_dashboard()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "User writes sample text in the message box",
  "keyword": "When "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.SendingMessageStepDefs.user_writes_sample_text_in_the_message_box()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "Clicks Send Button",
  "keyword": "And "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.SendingMessageStepDefs.clicks_Send_Button()"
});
formatter.result({
  "status": "passed"
});
formatter.step({
  "name": "Message is sent",
  "keyword": "Then "
});
formatter.match({
  "location": "com.CRM_7.stepDefinitions.SendingMessageStepDefs.messageIsSent()"
});
formatter.result({
  "status": "passed"
});
formatter.after({
  "status": "passed"
});
});